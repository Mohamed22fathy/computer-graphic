...
void timer(int v) {
	...
	if (eyez == 400 || eyez == 70 || eyez == 350 || eyez == 100) {
		score++;
	}
	else if (eyez == 200 || eyez == 300) {
		score = score + 2;
	}
	//COLLISIONS
	...
	else 	if (eyez >= 67 && eyez <= 75 && ((cxl >= squarex - 2 && cxl <= squarex) || (cxr >= -2 && cxr <= 0)) && flag == true) {
		start = 0;
		std::cout << "Game over \n";
		std::cout << "score: " << score;
		glutHideWindow();
...
void load(int imgnum) {
	if (imgnum == 1) {
		...
		data = stbi_load("side-view.jpg", &width, &height, &nrChannels, 0);
		check(data);
	}
	else if (imgnum == 2) {

		data = stbi_load("road.bmp", &width, &height, &nrChannels, 0);
		check(data);
	}
	else if (imgnum == 3) {
		...
		data = stbi_load("end_flag.png", &width, &height, &nrChannels, 0);
		check(data);
	}
	else if (imgnum == 5) {

		data = stbi_load("g.jpg", &width, &height, &nrChannels, 0);
		check(data);
	}
